...
export function BikeFitDialog({ children, equipment, userId, onSuccess }: BikeFitDialogProps) {
  ...
  const renderCleatField = (name: keyof CleatPosition, label: string, unit: string) => (
     <FormField
        control={form.control}
                        ...
                        <div className="space-y-3">
                           <h4 className="font-semibold">Cleat Position</h4>
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                {renderCleatField('foreAft', 'X: Fore-Aft', 'mm')}
                                {renderCleatField('lateral', 'Y: Lateral', 'mm')}
                                {renderCleatField('rotational', 'Z: Rotational', 'deg')}
                            </div>
                        </div>
